...
const DESKTOP_SIDE_PANEL_WIDTH = 370;
      ...
      {(showBuildingSheet || layout.isDesktop) && (
        <View
          style={[
            styles.sheetCard,
            styles.bottomSheet,
            layout.isDesktop && styles.sidePanel,
            layout.isDesktop && desktopMapHeight ? { height: desktopMapHeight } : null,
          ]}
        >
  ...
  const renderDesktopMapLayout = () => (
    <View>
      <View style={styles.topControls}>
        ...
        <View style={[styles.sideColumn, styles.toggleWrap]}>
          <SegmentedControl
            segments={["Map", "List"]}
            selectedIndex={viewMode}
            onChange={setViewMode}
            style={styles.segmentedDesktop}
          />
        </View>
      </View>

      <View style={styles.desktopMapRow}>
        <View
          style={[
            styles.mapFrame,
            styles.mapFrameDesktop,
            styles.primaryColumn,
            desktopMapHeight ? { height: desktopMapHeight } : null,
          ]}
        >
          <MapViewWrapper
            initialRegion={BYU_REGION}
    ...
    <PageShell>
      <View style={[styles.container, { paddingTop: topInset }]}>
        {layout.isDesktop ? (
          ...
          >
            {renderHeader()}
            {viewMode === 0 ? renderDesktopMapLayout() : (
              <>
                <View style={styles.topControls}>
                  <View style={styles.primaryColumn}>
                    <Pressable
                ...
                <View style={styles.desktopList}>{renderListContent()}</View>
              </>
            )}
          </ScrollView>
        ) : viewMode === 0 ? (
...
const styles = StyleSheet.create({
  ...
  sheetList: { flex: 1, paddingBottom: 24, paddingHorizontal: 16 },
  sheetEmpty: {
    fontSize: 14,
    fontFamily: "Inter_400Regular",
    color: Colors.light.textSecondary,
