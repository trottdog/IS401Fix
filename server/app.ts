...
function logRuntimeConfiguration(serveFrontend: boolean) {
  ...
  log(
    `frontend mode: ${
      serveFrontend ? "static web build served by Express" : "frontend served by host platform"
    }`,
  );
}
...
async function buildApp(options: CreateAppOptions = {}) {
  ...
  logRuntimeConfiguration(serveFrontend);
...
export async function createApp(options: CreateAppOptions = {}) {
  const useCache = options.serveFrontend === false;
  ...
  if (!cachedAppPromise) {
    cachedAppPromise = buildApp(options);
  }
